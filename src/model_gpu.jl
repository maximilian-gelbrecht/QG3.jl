...
"""
TR(ψ::AbstractArray{T,3}, m::QG3Model{T}) where T<:Number = cuda_used[] ? reshape(batched_vec(m.TR_matrix, reshape(ψ,3,:)),3 , m.p.N_lats, m.p.N_lons) : reshape(batched_vec(m.TR_matrix, reshape(ψ,3,:)),3 , m.p.L, m.p.M)
